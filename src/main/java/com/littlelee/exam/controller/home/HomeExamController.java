...
import com.littlelee.exam.entity.admin.*;
...
public class HomeExamController {
    ...
    public Map<String ,String> startExam(Long examId, HttpServletRequest request) {
        ...
        if (find != null) {
            if (find.getStatus() == 1) {
                //表示考试已经考过
                result.put("type", "error");
                result.put("msg", "您已经考过这门考试，不能再考！");
                return result;
            }
            //走到这里表示试卷已经生成，但是没有提交考试，可以开始重新考试
            result.put("type", "success");
            result.put("msg", "可以开始考试");
            return result;
        }
        //此时，说明符合考试条件，随机生成试卷
        //要做判断，看是否满足生成试卷的条件
        ...
        qMap.put("questionType", (long) Question.QUESTION_TYPE_SINGLE);
        ...
        if (exam.getMuiltQuestionNum() > 0) {
            ...
            List<Question> selectedMuiltQuestionList = this.getRandomList(muiltQuestionList, exam.getMuiltQuestionNum());
    ...
    public ModelAndView index(ModelAndView model,Long examId,HttpServletRequest request){
        ...
        Integer minute = (exam.getAvailableTime()-passedTime)%60;
